...
run_with_uv() {
    ...
    # If it's a relative path starting with src/, make it absolute
    if [[ $script_path == src/* ]]; then
        script_path="${BASE_DIR}/${script_path}"
    fi
    
    echo -e "\n${YELLOW}=== Running: $script_path ===${NC}"
...
view_status() {
    ...
    case $status_choice in
        1)
            ...
            ps aux | grep -E "src/process_links.py|src/run_youtube_post.py|src/run_instagram_post.py" | grep -v grep
            ;;
        2)
...
run_diagnostics() {
    ...
    case $diag_choice in
        1)
            ...
            run_with_uv src/check_notion_connection.py
            ;;
        2)
            echo -e "${BLUE}Checking Instagram connectivity...${NC}"
            run_with_uv src/check_instagram_connection.py
            ;;
        3)
...
manage_config() {
    ...
    case $config_choice in
        1)
            ...
            if [ -f "src/platform_config.py" ]; then
                echo -e "${YELLOW}=== Platform Settings ===${NC}"
                grep -E "YOUTUBE_|INSTAGRAM_|DEFAULT_" src/platform_config.py
            else
        ...
        2)
            ...
            grep -E "YOUTUBE_" src/platform_config.py
            ...
            sed -i "s/YOUTUBE_BATCH_SIZE = [0-9]*/YOUTUBE_BATCH_SIZE = $yt_batch/" src/platform_config.py
            sed -i "s/YOUTUBE_DELAY_SECONDS = [0-9]*/YOUTUBE_DELAY_SECONDS = $yt_delay/" src/platform_config.py
        ...
        3)
            ...
            grep -E "INSTAGRAM_" src/platform_config.py
            ...
            sed -i "s/INSTAGRAM_BATCH_SIZE = [0-9]*/INSTAGRAM_BATCH_SIZE = $ig_batch/" src/platform_config.py
            sed -i "s/INSTAGRAM_DELAY_SECONDS = [0-9]*/INSTAGRAM_DELAY_SECONDS = $ig_delay/" src/platform_config.py
...
process_youtube() {
    ...
    if [ -f "src/process_links.py" ]; then
        run_with_uv src/process_links.py --platform youtube --limit "$limit"
    else
        # Fallback to old script if new one doesn't exist
        run_with_uv src/run_youtube_post.py --limit "$limit"
    fi
...
process_instagram() {
    ...
    if [ -f "src/process_links.py" ]; then
        run_with_uv src/process_links.py --platform instagram --limit "$limit"
    else
        # Fallback to old script if new one doesn't exist
        run_with_uv src/run_instagram_post.py --limit "$limit"
    fi
...
process_all() {
    ...
    if [ "$parallel" = "true" ]; then
        ...
        if [ -f "src/process_links.py" ]; then
            echo -e "${YELLOW}=== Running: process_links.py --platform all --limit $limit --parallel ===${NC}"
            # Use the current directory to ensure the script is found
            cd "$(dirname "$0")" && uv run python src/process_links.py --platform all --limit "$limit" --parallel
        else
            # Fallback to old scripts if new one doesn't exist
            echo -e "${YELLOW}=== Running individual platform processors in parallel ===${NC}"
            # Make sure we're in the right directory and properly reference the scripts
            cd "$(dirname "$0")"
            uv run python src/run_youtube_post.py --limit "$limit" &
            youtube_pid=$!
            uv run python src/run_instagram_post.py --limit "$limit" &
            instagram_pid=$!
            
            # Wait for both processes to complete
            echo -e "${YELLOW}Waiting for processors to complete...${NC}"
            wait $youtube_pid $instagram_pid
        fi
    else
        ...
        if [ -f "src/process_links.py" ]; then
            run_with_uv src/process_links.py --platform all --limit "$limit"
        else
            # Fallback to old scripts if new one doesn't exist
            run_with_uv src/run_youtube_post.py --limit "$limit"
            run_with_uv src/run_instagram_post.py --limit "$limit"
...
# Main menu function
show_menu() {
    # Check content viewer status for display
    if is_viewer_running; then
        viewer_status="${GREEN}[RUNNING]${NC}"
    else
        viewer_status="${RED}[STOPPED]${NC}"
    fi
    ...
    echo -e "${YELLOW}7)${NC} Start content viewer ${viewer_status}"
    echo -e "${YELLOW}8)${NC} Stop content viewer"
    echo -e "${YELLOW}9)${NC} Restart content viewer"
        ...
        case "$1" in
            ...
            "viewer")
                if [ "$2" = "start" ]; then
                    ...
                    echo -e "${YELLOW}Usage: $0 viewer [start|stop|restart|status]${NC}"
                fi
                ;;
            *)
